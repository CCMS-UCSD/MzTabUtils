	...
	public static void main(String[] args) {
		...
		for (File file : files) {
			...
			reader.addProcessor(new SpectraRefValidationProcessor(
				validation.uploadedResultDirectory, validation.scansDirectory));
			Map<String, Integer> counts = new HashMap<String, Integer>(2);
			reader.addProcessor(new PSMValidationProcessor(counts));
			// validate file
			try { reader.read(); }
			catch (RuntimeException error) {
				// if a RuntimeException is caught, and its parent is an
				// UnverifiableNativeIDException, then that means that the
				// validator tried to interpret ambiguous nativeIDs as scan
				// numbers but failed somewhere along the way; in this case,
				// try again with a hard-coded interpretation scheme of indices
	...
	private static MzTabValidationOperation extractArguments(String[] args) {
		...
		try {
			return new MzTabValidationOperation(params, mzTabDirectory,
				...
				scansDirectory, resultDirectory, output, failureThreshold);
